...
Good examples: "Observability System for Mota Framework", "Multi-tenant Workspace Support"
-->
...
This RFC proposes creating a tutorial for Mota that will help users understand how to use Mota's ecosystem (framework, workbench, cloud). It is aimed to be supplemental to the existing documentation and provide a more hands-on approach to learning Mota.

## Background

Currently, the learning curve for Mota is high due to the constant evolution of the Mota ecosystem. Most developers come with a background of what they would expect Mota does, depending on the marketing material they have seen and how they associate it to other products.

The main root causes that this RFC addresses are:

- Disarray of the documentation due to the constant evolution of the Mota ecosystem
- Diversity of templates that showcase different use cases of the Mota framework (i.e.: event based parallelism, scheduled tasks, etc...)
- Lack of hands-on examples that help developers understand how to use Mota ecosystem

## Goals

### Primary Goals

1. Create a tutorial that helps users understand how to use Mota ecosystem, provided through the `motia create` cli command
2. Provide a set of templates that showcase different use cases of the Mota framework primitives
...
- Tutorial to use Mota cloud, this would require a terminal, we need to evaluate what would be the bext devex experience for this and would be best suited as a separate initiative (i.e. one click deployment from workbench without the need of a cli)
...
```mermaid
sequenceDiagram
    Developer->>+Mota CLI: `npx motia@latest create`
    Mota CLI->>+Developer: Do you wish to learn how to use Mota through a tutorial? Y/N
    Developer-->>-Mota CLI: Y
    Mota CLI-->>-Developer: open browser
    Note left of Mota CLI: create project with flow templates <br /> for tutorial and open browser with workbench
    Mota CLI-->>+Workbench: open workbench with first templatate of the tutorial
    rect rgb(42, 126, 254)
        loop Mota Tutorial
...
The following is an example from driver.js, the final layout would be similar but themed to match Mota's design language.
...
### Example 1: As a developer, I want to start a new motia project and learn how to use the Mota ecosystem

```
> npx motia@latest create
> Do you wish to learn how to use Mota through a tutorial? Y/N
> Y
> ... rest of the create process
> Your project is ready, you will be redirect to Mota Workbench to start the tutorial
```
...
- Changes to the Mota ecosystem will require changes to the tutorial

### Risk Assessment

- Given the constant evolving stage of Mota, the tutorial might need to be updated frequently and it can put additional strains on new feature development, being a potential cause to degrade the quality of the tutorial experience.
...
- **Cons**: Easy to ge out of sync and keep up to date with the evolving state of the Mota ecosystem, in some cases it can become expensive, it can be less engaging for some developers that want a hands on experience. lack of control over the content consumption which can conduce to users consuming outdated information.
- **Decision**: This is an add-on for documentation but not a replacement for an interactive tutorial. Better suited to announce new features and provide a high level overview of the Mota ecosystem.

### Alternative 2: Examples

- **Pros**: Diversitry in use cases implemented with Mota that can cast a wide net of developers with different backgrounds (enterprise vs startup, architect vs senior/entry level, etc...)
...
- Integrate Mota Cloud as a segment in the tutorial, we need to find a way to have a one click deploy from Workbench to Mota Cloud
...
Reiterate why this proposal is important for Mota and its users.
-->

This proposal is important for Mota and its users (developers) because it will provide a hands-on experience for developers to learn how to use Mota's ecosystem, through a step by step guide that outlines a selection of templates that demonstrate how to use Mota's primitives and tools.
